...
\begin{claim}
(Funcoidal) join~$L$ of one-si\-de vicinities (laying on great circles) of each point on $S^n_{>}$ hemisphere has dimension~$n$, provided that the set~$K_1$ of directions (measured as their ``projection'' to unit sphere) is $n$-di\-men\-sio\-nal.
\end{claim}

Replace each ``curved'' vicinity from point~$a$ of the hypersphere by a straight one~$h_a$ (going outside the hypersphere in~$\mathbb{R}^{n+1}$). Let $s_a$ be an ``outward'' radial vicinity of point~$a$. Then take funcoidal join $H=\bigsqcup_{a\in K_1}(h_a\sqcup s_a)$. Informally, $H$ is a ``swelling'' on the hypersphere in $\mathbb{R}^{n+1}$.

The straight vicinities $h_a$, projected to the sphere by the radial projection, become exactly original curved vicinities (because they lay on great circles).

If $H$ projected to the sphere (by straight lines toward its center) has dimension~$n$, then the claim is proved, because it is obviously equal to~$L$.
