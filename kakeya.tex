...
\begin{lem}
If $a$,~$b$ are ultrafilters on~$\mathbb{R}$ and $k\in\mathbb{R}$, then:
\begin{enumerate}
\item $ka$ is an ultrafilter;
\item $a+b$ is an ultrafilter.
\end{enumerate}
\end{lem}

\begin{proof}
A defining property for~$x$  to be an ultrafilter is $\forall X:(X\in x\Leftrightarrow{\uparrow}X\intrs x)$.
\begin{enumerate}
\item $X\in ka \Leftrightarrow \frac{X}{k}\in a\Leftrightarrow \mathord{\uparrow}\frac{X}{k}\intrs a \Leftrightarrow \mathord{\uparrow}X\intrs ka$.
\item $\mathord{\uparrow}X\intrs a+b \Leftrightarrow \forall I\in a,J\in b:X\intrs I+J \Leftrightarrow \forall I\in a,J\in b\exists i\in I,j\in J:i+j\in X$.
\item $\mathbb{R}\setminus X\in a+b \Leftrightarrow \nexists x\in X:x\in a+b \Leftrightarrow
\nexists x\in X\exists I\in a,J\in b:x\in I+J \Leftrightarrow \lnot \exists I\in a,J\in b\exists x\in X:x\in I+J \Leftrightarrow
\lnot \exists I\in a,J\in b:X\cap(I+J)\ne\emptyset \Leftrightarrow \forall I\in a,J\in b:X\cap(I+J)=\emptyset \Leftrightarrow
$
.
??
\end{enumerate}
...
\begin{equation}\label{rotated-funcoid}
...
\[ x_0' = x_0\cos\alpha - x_1\sin\alpha,\quad x_1' = x_0\sin\alpha + x_1\cos\alpha. \]
So, let \[ x' = (x_0',x_1',x_2,\dots,x_{n-1}). \]
If $x_0$,~$x_1$ are ultrafilters, then $x_0\cos\alpha$ and $x_1\sin\alpha$ are obviously ultrafilters. So, $x_0'$~is an ultrafilter because[TODO: Use lemma.] \[ X\intrs x_0\cos\alpha - x_1\sin\alpha \Leftrightarrow \forall I\in x_0\cos\alpha, J\in x_1\sin\alpha:X\intrs I-J?? \]
??.
Define $\delta'$ by the formula $x'\in\delta'\Leftrightarrow x\in\suprel{f}$. Then
\begin{multline*}
x'\in\delta' \Leftrightarrow \forall X\in f,i=\{0,\dots,n-1\}:x'\intrs\mathord{\uparrow}X_i\Leftrightarrow??\\
\forall X\in f,i=\{0,\dots,n-1\}:x'\in\atoms X_i.
\end{multline*}
% \[ \delta' = \setcond{(\lambda i\in\{0,\dots,n-1\}:P[x_i])}{x\in\suprel{f}}. \]
% Then??
% \[ \delta' = \setcond{(\lambda i\in\{0,\dots,n-1\}:x_i)}{x\in\suprel{f}}. \]
To prove the formula~\eqref{rotated-funcoid} and uniqueness of~$f'$ it is enough to prove that~$f'$ conforms to the conditions of the theorem~\ref{v-on-atoms}.

Then\fxerror{$f'$ is not yet defined.}
\begin{multline*}
(\lambda i\in\{0,\dots,n-1\}:P[x_i])\in\suprel{f'} \Leftrightarrow\\
x_0'\times^{\funcoids}x_1'\times^{\funcoids}x_2\times^{\funcoids}\dots\times^{\funcoids}x_{n-1} \in \suprel{f'}
\end{multline*}
??
\end{proof}
