...
\title{A Proof of Kakeya's Conjecture (rough draft)}
...
\begin{claim}
Union~$L$ of one-si\-de intervals in~$\mathbb{R}^n$ has dimension~$n$, provided that the set~$K$ of directions of the intervals is $n$-di\-men\-sio\-nal.
\end{claim}

Using that $\mathbb{R}^n$ is diffeomorphic to~$S^n_{>}$, we can instead prove the claim for~$S^n_{>}$:

\begin{claim}
Union~$L$ of one-si\-de arcs $v$ in~$S^n_{>}$ hemisphere has dimension~$n$, provided that the set~$K'$ of directions (measured as their ``projection'' to unit sphere) is $n$-di\-men\-sio\-nal.
\end{claim}
...
\fxwarning{Need to modify pictures, because meaning of~$h_v$ has changed.}
Replace each ``curved'' vicinity $v$ by the funcoid~$h_v=z^{-1}v$  (a ``sector'' in a hyperplane). Then take the union $H=\bigcup_{a\in K'}h_v$.
...
The funcoids $h_v$, projected to the sphere by the radial projection, become exactly original curved vicinities $v$: $\Lambda h_v=v$, where $\Lambda$ is the radial projection to the hypersphere.
