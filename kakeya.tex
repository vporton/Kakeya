...
\title{[ERRONEOUS] A Proof of Kakeya's Conjecture}
...
\url{https://www.academia.edu/90158933/Equivalences_between_different_forms_of_the_Kakeya_conjecture_and_duality_of_Hausdorff_and_packing_dimensions_for_additive_complements}
...
Let's draw (see figure~\ref{fig:filters}) an arc~$v$ of the unit hypersphere. Let it passes through point~$a_v$ and has direction~$d_v$.
...
\begin{cor}
Let~$H$~be a set in our space $\mathbb{R}^{n+1}$, then the dimensionality of its radial (toward the center of the hypersphere) projection~$K'$ to~$S^n$ $\dim K' \geq \dim H - 1$.
\end{cor}

\begin{proof}
Apply to~$K'$ the reverse of the projection function~$z^{-1}$. The result is $H'=z^{-1}[K']\supseteq H$ and therefore $\dim(K'\times\mathbb{R})=\dim H'=n+1$. Therefore $\dim K' = \dim H' - 1 \geq \dim H - 1$.
...
\begin{proof}
Because~$\Lambda$~is a Lipshitz map.
\end{proof}

So, it remains to prove that $\dim L \geq n+1$.

Consider the tangent line $c_v(t)$ corresponding to the direction~$h_v$ that is \[ c(t) = a_v + t d_v. \]

Then by \url{https://grok.com/share/bGVnYWN5_3cb86ec3-4870-4efc-bde0-0510ab63ab02} \[ b_v(t) = \Lambda c_v(t) = \left(1-\frac{1}{\sqrt{1+t^2}}\right)(a_v + t d_v). \]

\fxwarning{Proof not finished.}
So, it seems, I've just reduced the problem to more complex problem.

===

??We can?? also exchange the position and the direction on the hypersphere. Does this employ projective geometry?

It looks like, I found a simple proof strategy even without using~$h_v$ and~$H$:
First limit the size (more formally, diameter) of the set to some finite one. This way we limit how much the great circle may be slope. If we also take each $v$ long enough to touch e.g.\ the North of the hypersphere and some (fixed) latitude, then rotating the great circle, we get some area/volume of the hypersphere. It is enough to prove.
If we want to reduce the radius futher, we can split the hypercube with the figure inside into $2^n$~parts and take one of them.
