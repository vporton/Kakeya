...
\fxnote{See the \href{https://grok.com/share/bGVnYWN5_3cb86ec3-4870-4efc-bde0-0510ab63ab02}{this AI musings}.}
...
Thus we have enough to prove that the join~$bH$ of one-si\-de vicinities~$w_v$ for a full-me\-a\-su\-re choice~$d$ of directions of the same point (the center of our sphere) is~$n$. \fxerror{They are not directions, they are points~$a$ that are not necessarily full measure.}

It's easy to show~(\cite{curve}) that
\[ b(x(y)) = \left(\sqrt{1+t^2\norm{v^2}}-1\right)\frac{a+tv}{\sqrt{1+t^2\norm{v^2}}} \]
where \fxwarning{Duplicate notation.}
$x$~is the tangent line, $v$ is a vector of direction of vicinity~$v$.
...
Solve?? $b(x_a(\epsilon)) = \sum_{k\in\mathbb{N}}a_k 3^{-k}$, where $b(x(0)) = \sum_{k\in\mathbb{N}}\frac{a_k}{2} 2^{-k}$.

Then?? for $0<t<\epsilon$ we have $b(x_a(t))$ is a Lipshitz distortion~\cite{Cantor-distort}) of Cantor set and therefore has measure zero.
