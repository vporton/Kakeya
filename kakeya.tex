...
\begin{claim}
(Funcoidal) join of one-si\-de vicinities of each point on $S^n\setminus\{\zeta\}$ has dimension~$n$, provided that the set~$K_1$ of directions (measured as their ``projection'' to unit sphere) is $n$-di\-men\-sio\-nal. [FIXME: North pole?]
\end{claim}
...
Let $H=\bigsqcup_{a\in??}h_a$ be the funcoidal join of all~$h_a$.

We've got a swollen hypersphere in $\mathbb{R}^{n+1}$.

If $H$ projected to the sphere (by straight lines toward its center) has dimension~$n$, then the claim is proved, because ??.
...
By properties of diffeomorphisms??, the shrinked swelling has (deformed) vicinities of a set of directions of full dimension.
