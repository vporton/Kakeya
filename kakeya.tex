...
\textbf{It seems that my proof attempt failed:} $\supfun{b}h_a$ isn't on a straight line, every ``hair'' is on its own curve, so not creating an entire vicinity of hypersphere center. So, it is not proved that the join of all~$\supfun{b}h_a$ has dimension~$n+1$.

\textbf{The proof probably can be salvaged} by replacing each $h_a$ by the (funcoidal) join of $h_a$ and a radial one-side vicinity.
...
\begin{lem}
$\supfun{b}h_a$ is a vicinity from the center to the direction~$a$. [FIXME: The resulting vicinity isn't on a straight line. This seems to create a big problem.]
\end{lem}
