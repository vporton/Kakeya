...
\begin{cor}
[??It seems, it is~$n+1$, not~$n$.]
\fxwarning{It's impossible to disprove Kakeya this way, because the reverse implication does not hold.
So, if I proved $H$ has lesser dimension than~$n+1$, it does not disprove Kakeya conjecture.}
...
It follows, that if we prove that $H$ has dimension~$n+1$, then $L$~has dimension~$n$ and our claim is proved.\fxnote{Prove $\dim H=n+1\Leftrightarrow\dim L=1$ using Fubini's theorem, instead. This, probably, makes no sense because by the same theorem I could direct vicinities orthogonally.}
...
It seems I proved a simple proof strategy even without using~$h_v$ and~$H$:
